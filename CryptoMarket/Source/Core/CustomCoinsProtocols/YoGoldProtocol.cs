...
namespace CryptoMarket.Source.Core.CustomCoinsProtocols {
    ...
    public class YoGoldProtocol {
        ...
        public YoGoldProtocol(ApplicationDbContext context) {
            _dbContext = context;
        ...
        public async Task<string> Transfer(string addressTo, double amount) {
            ...
            return await _web3.Eth.GetContract(YoGoldABI, YoGoldContractAddress).GetFunction("transfer").SendTransactionAsync(CoinbaseAccount, null, null, addressTo, _web3.Convert.ToWei(amount) / 100);
        }
        ...
        public static void RegisterTransferFilter() {
            LogManager.WriteToEventLog($"YoGold filter creating");
            _filterHex = new Web3("http://93.190.142.88:8545").Eth.GetContract(YoGoldABI, YoGoldContractAddress).GetEvent("Transfer").CreateFilterAsync().Result;
            LogManager.WriteToEventLog($"YoGold filter created: {_filterHex.Value}");
        }

        /// <summary>
        /// 
        /// </summary>
        public class DepositYoGJob : IJob {
            public void Execute(IJobExecutionContext contextExe) {

                foreach (var transferRes in (GetEvents<TransferEvent>(YoGoldContractAddress, YoGoldABI, "Transfer", _filterHex)).Result.ToArray()) {
                    LogManager.WriteToEventLog($"New YOG Event on:{transferRes.AddressTo}, amount:{transferRes.Value}");
                    ...
                    using (var context = new ApplicationDbContext()) {
                        ...
                        var yocoinCoin = context.CoinSystems.First(_ => _.ShortName == "YOG");
